...
import gov.nih.nci.evs.cdisc.report.HtmlReportGenerator;
import gov.nih.nci.evs.cdisc.report.IchHtmlReportGenerator;
...
public class LambdaHandler implements RequestHandler<ReportSummary, ReportSummary> {
  ...
  /**
   * Creates HTML reports from the ODM XML file created by odm-report. Uses XSLT to transform the
   * XML to HTML. This lambda creates a standalone HTML report and an HTML report that will be
   * converted in PDF in another process
   *
    ...
    for (ReportDetail reportDetail : input.getReportDetails()) {
      ...
      if (odmXmlFile != null && new File(odmXmlFile).exists()) {
        ...
        if (odmXmlFile.toLowerCase().contains("ich")) {
          log.info("Generating ICH HTML reports for {}", odmXmlFile);
          generator = new IchHtmlReportGenerator();
        } else {
          log.info("Generating CDISC HTML reports for {}", odmXmlFile);
          generator = new HtmlReportGenerator();
        }
        reportDetail
                .getReports()
                .put(
                        ReportEnum.MAIN_HTML,
                        generator.generateHtmlReport(odmXmlFile, ReportEnum.MAIN_HTML));
        reportDetail
                .getReports()
                .put(
                        ReportEnum.PDF_HTML,
                        generator.generateHtmlReport(odmXmlFile, ReportEnum.PDF_HTML));
      } else {
