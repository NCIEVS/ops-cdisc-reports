...
public class ChangesReport {
  ...
  public static void printHelp() {
    ...
    System.out.println(
        "Unable to start program due to improper input parameters. Program will exit.");
    System.out.println();
    System.out.println(
        "Run as: DiffCDISC [new report text file] [previous report text file] [release date] [output filename]");
    System.out.println(
        "Example: DiffCDISC \"SDTM Terminology.txt\" \"SDTM Terminology 2014-03-28.txt\" \"2014-06-27\" \"D:\\data\\SDTM Changes.txt\"");
    System.out.println();
    System.exit(0);
  }
  ...
  public void compareElements(
      ArrayList<Element> els_new, ArrayList<Element> els_old, String clShortName) {
    ...
    if (!newElement.getSubValue().equals(oldElement.getSubValue()))
      changes.add(
          new Change(
              ...
              oldElement.getSubValue(),
              newElement.getSubValue()));
    if (!newElement.getDefinition().equals(oldElement.getDefinition()))
      changes.add(
          new Change(
              ...
              oldElement.getDefinition(),
              newElement.getDefinition()));
    ...
    if (!cl_new.getExtensible().equals(cl_old.getExtensible()))
      changes.add(
          new Change(
              ...
              "Update",
              cl_new.getCode(),
              "CDISC Extensible List",
              cl_new.getSubValue(),
              cl_new.getName(),
              "Update CDISC Extensible List",
              cl_old.getExtensible(),
              cl_new.getExtensible()));
    ...
    if (!cl_new.getDefinition().equals(cl_old.getDefinition()))
      changes.add(
          new Change(
              ...
              cl_new.getName(),
              "Update CDISC Definition",
              cl_old.getDefinition(),
              cl_new.getDefinition()));
    ...
    try {
      ...
      while (input.hasNext()) {
        ...
        if (tokens.length == 8) {
          ...
          if (!code.equals("") && codelistCode.equals("")) {
            Codelist cl =
                new Codelist(code, extensible, codelistName, subValue, synonyms, definition, pt);
            map.put(cl, null);
          } else {
            Element e =
                new Element(code, codelistCode, codelistName, subValue, synonyms, definition, pt);
            boolean found = false;
